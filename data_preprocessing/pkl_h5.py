...
class PKL2H5:
    ...
    def _get_flux(self):
        grab_flux = []
        key_name = 'flux'
        for dir in tqdm(self.filepath_list, desc='   - Getting flux data for all astro objects', total=len(self.filepath_list)):
            with open(dir, "rb") as openfile:
                load = pickle.load(openfile)
        ...
        with h5py.File(os.path.join(self.h5_output, self.h5_name), 'r+') as f:
            try:
                f.create_dataset(key_name, data=np.array(
                    grab_flux).astype(float))
                assert key_name in list(f.keys())
                self.logger.debug(
                    f"Added key '{key_name}' as a HDF5 root level key")
            except h5py.H5FError as e:
                self.logger.error(f"HDF5 file error: {e}")
            except h5py.Dataset.DatasetError as e:
                self.logger.error(f"Dataset error for key '{key_name}': {e}")
            except KeyError:
                self.logger.error(f"Key '{key_name}' not found after creation")
            except ValueError as e:
                self.logger.error(f"Value error while creating dataset: {e}")
            except RuntimeError as e:
    ...
    def _get_mask(self):
        grab_mask = []
        key_name = 'mask'
        for dir in tqdm(self.filepath_list, desc='   - Getting mask data for all astro objects', total=len(self.filepath_list)):
            with open(dir, "rb") as openfile:
                load = pickle.load(openfile)
                grab_mask.append(load[key_name])

        with h5py.File(os.path.join(self.h5_output, self.h5_name), 'r+') as f:
            try:
                f.create_dataset(key_name, data=np.array(
                    grab_mask).astype(float))
                assert key_name in list(f.keys())
                self.logger.debug(
                    f"Added key '{key_name}' as a HDF5 root level key")
            except h5py.H5FError as e:
    ...
    def _get_time(self):
        grab_time = []
        key_name = 'time'
        for dir in tqdm(self.filepath_list, desc='   - Getting time data for all astro objects', total=len(self.filepath_list)):
            with open(dir, "rb") as openfile:
                load = pickle.load(openfile)
                grab_time.append(load[key_name])

        with h5py.File(os.path.join(self.h5_output, self.h5_name), 'r+') as f:
            try:
                f.create_dataset(key_name, data=np.array(
                    grab_time).astype(float))
                assert key_name in list(f.keys())
                self.logger.debug(
                    f"Added key '{key_name}' as a HDF5 root level key")
            except h5py.H5FError as e:
    ...
    def _get_ft(self):
        ...
        grab_ft = []
        for dir in tqdm(self.filepath_list,
                        desc='   - Getting feature data for all astro objects',
                        total=len(self.filepath_list)):
            with (open(dir, "rb")) as openfile:
                load = pickle.load(openfile)
    ...
    def _get_md(self):
        # TODO check!
        grab_md = []
        for dir in tqdm(self.filepath_list, desc='   - Getting meta data for all astro objects', total=len(self.filepath_list)):
            with (open(dir, "rb")) as openfile:
                load = pickle.load(openfile)
    ...
    def _create_ft_fold(self, seed, prefix=''):
        ...
        # print(feats)
        # TODO insert prefix variable in dump filename
