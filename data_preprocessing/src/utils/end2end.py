...
from ..lc_classifier.lc_classifier.utils import create_astro_object
...
def transform2arrays(aos_list,config_dict,ft_ex, lc_ex):
    ...
    for ao in aos_list:
        ...
        ao.features["name_fid"] = ao.features['name'] + ao.features['fid'].apply(lambda x: '_' + x if x != '' else '')
        ...
        metadata = ao.features[ao.features['prod'].isin(
            config_dict["md_cols"])]
        #print('features:',ao.features.shape)
        #print('metadata:',ao.metadata.shape)
        assert features.shape[0] == len(config_dict['feat_cols'])
        assert metadata.shape[0] == len(config_dict['md_cols'])
