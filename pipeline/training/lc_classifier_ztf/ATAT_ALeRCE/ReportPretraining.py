...
class Report:
    ...
    def _init_dataloader(
        ...
        if sampler is not None:
            modded_config.datamodule.train_use_sampler = False
    ...
    def _predict(self,dataloader, modality):
        ...
        for b1 in tqdm(dataloader):
            ...
            t = b1["labels"]
            save_tuple = torch.count_nonzero(b1['data'], dim = 1)

            self.count_nonzero = (
                np.concatenate([self.count_nonzero, save_tuple.detach().cpu().numpy()])
                if self.count_nonzero is not None
                else save_tuple.cpu().detach().numpy()
            )
...
class ReportZeroshot(Report):
    ...
    def fit_knn(self, dataset_type ='training', modality = 'LC', sampler= False):
        from sklearn.metrics import classification_report
        dataloader = self._init_dataloader(
        ...
        self.knn_model.fit(preds_out,target)
    ...
    def pred_knn(self, dataset_type ='validation', modality = 'LC', sampler= False):
        ...
        for b1 in tqdm(dataloader):
            ...
            lc_emb = self.model.predict_embedding(**b1)  # [:, 0, :]
            preds_out = (
                np.concatenate([preds_out, lc_emb.detach().cpu().numpy()])
                if preds_out is not None
                else lc_emb.cpu().detach().numpy()
