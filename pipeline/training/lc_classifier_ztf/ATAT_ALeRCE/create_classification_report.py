...
class ReportClassification:
    ...
    def _init_dataloader(self, path_to_dataset, set_type="test", seed=0, batch_size=128):
    ...
    def get_classification_report(self):
        rep = classification_report(test.preds_out, test.target, target_names=list(DICT.keys()),digits = 4, output_dict = True)
    ...
    def get_confusion_matrix(self):
        ...
        out_metrics_balto = classification_report(
            self.target, self.preds_out, target_names=list(self.class_dict.keys()), output_dict=True
        )["macro avg"]
        template_balto = ""
        for key in out_metrics_balto.keys():
            template_balto += " {} : {:.3f} ".format(key.upper(), out_metrics_balto[key])
        elasticc_confusion_matrix(
            y_true=np.array(self.target).astype(int),
            y_pred=np.array(self.preds_out).astype(int),
            classes= np.array(list(self.class_dict.keys())),
            ax=axes,
            normalize=True,
...
def create_classification_report_pdf( 
    ...
    test = ReportClassification(
            path_to_training_dir='/home/mdelafuente/pipeline/pipeline/training/lc_classifier_ztf/ATAT_ALeRCE/results/ZTF_ff/LC/pretrained_v4windows_0/',
            path_to_dataset= 'data/datasets/ZTF_ff/final/LC_MD_FEAT_240627_windows_200_12',
            model_class = LightCurveClassifier,
            model_type = 'lc',
            device = 'cuda:2',
            seed = 0)
    ...
    #report = classification_report(test.target, test.preds_out, target_names=list(test.class_dict.keys()),digits = 4)
    
    # Format the report as a paragraph with monospace font

    report_style = ParagraphStyle('ReportStyle', parent=styles['Normal'], fontName='Courier')
    report_paragraph = Paragraph(f"<pre>{classification_report(test.target, test.preds_out, target_names=list(test.class_dict.keys()),digits = 4)}</pre>", report_style)
    ...
    test.get_confusion_matrix()#test._2d_umap_plot(15, 0.1, "euclidean", test.preds_out, test.target, "test")
    plt.tight_layout()
